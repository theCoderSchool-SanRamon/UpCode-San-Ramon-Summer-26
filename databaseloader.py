...
if len(argv) <= 1:
	targetFile = input("Enter file to load: ").replace("\\", "/")
else:
	targetFile = argv[1].replace("\\", "/")
if targetFile.endswith("json"):
	print("Attempting to open Census-formatted JSON.")
	with open(targetFile) as file:
		dataframe = pandas.read_json(file, orient="records")
...
if "county" in dataframe.columns:
	...
	cursor.execute(
		f"UPDATE Costs SET {preferred} = (SELECT Loading.{columnname} FROM Loading WHERE Loading.state = Costs.state AND Loading.county = Costs.county)"
	)
	...
	cursor.execute(
		f"UPDATE PlaceAttributes SET {preferred} = (SELECT Loading.{columnname} FROM Loading WHERE Loading.state = PlaceAttributes.state AND Loading.place = PlaceAttributes.place)"
	)
