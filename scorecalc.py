...
counties = {}
for countyname, statename, houseprice, rent, rent2018, propertytax, vacantforrent, rentedunoccupied, renteroccupied, population, populationchange in rows:
	key = f"{countyname}_{statename}"

	ratio = houseprice / (rent * 12) if valid(houseprice) and valid(rent) and rent > 0 else None
	apprec = populationchange / population if population and population > 0 and populationchange is not None else None
	rentGrowth = (rent - rent2018) / rent2018 if valid(rent) and valid(rent2018) and rent2018 > 0 else None
	tax = propertytax / houseprice if valid(propertytax) and valid(houseprice) and houseprice > 0 else None
	vacDenom = (renteroccupied or 0) + (vacantforrent or 0) + (rentedunoccupied or 0)
	vac = (vacantforrent / vacDenom
		if valid(vacantforrent) and valid(rentedunoccupied) and valid(renteroccupied) and vacDenom > 0
		else None)

	counties[key] = {"ratio": ratio, "apprec": apprec, "rentGrowth": rentGrowth, "tax": tax, "vac": vac}
...
for key, factors in counties.items():
	...
	for name, cfg in FACTORS.items():
		raw = factors[name]
		if raw is None or name not in ranges: continue
		r = ranges[name]
		v = min(max(raw, r["p5"]), r["p95"])
		if r["max"] == r["min"]:
			norm = 1.0
		else:
			norm = (r["max"] - v) / (r["max"] - r["min"]) if cfg["invert"] else (v - r["min"]) / (r["max"] - r["min"])
	...
	if weight_present > 0:
		...
		factors["partialData"] = weight_present < sum(f["weight"] for f in FACTORS.values()) - 1e-9
	else:
		factors["score"] = None
...
print(f"Scored {sum(1 for c in counties.values() if c.get('score') is not None)} of {len(countyProperties)} counties.")
