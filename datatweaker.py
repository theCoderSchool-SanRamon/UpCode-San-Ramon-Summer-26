...
class DataTweaker():
	...
	def __init__(self):
		...
		with open("countydata.js") as file: self.data = json.loads(file.readline()[24:])
	...
	def insert_data(self, position: str, query: str):
		...
		if not query in [x[1] for x in self.cursor.execute("PRAGMA table_info(Costs)").fetchall()]: raise AttributeError(name=query, obj=self)
		position_path = position.split('.')
		for i in self.data.keys():
			self.cursor.execute(f"SELECT \"{query}\" FROM Costs WHERE Costs.countyname = ? AND Costs.statename = ?", (i.split('_')[0], i.split('_')[1]))
			item = self.cursor.fetchone()
			if item != None: item = item[0]
			exec(f"self.data[\"{i}\"]" + ''.join(["[\"" + x + "\"]" for x in position_path]) + " = item")
	...
	def insert_subdict(self, position: str):
		...
		position_path = position.split('.')
		for i in self.data.keys():
			exec(f"self.data[\"{i}\"]" + ''.join(["[\"" + x + "\"]" for x in position_path]) + " = {}")
