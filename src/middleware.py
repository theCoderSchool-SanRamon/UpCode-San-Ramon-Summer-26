...
def load_local_headers():
	with sqlite3.connect("census.db") as conn:
		...
		county_headers = [x[1] for x in cursor.execute("PRAGMA table_info(Costs)").fetchall()]
		global place_headers
		place_headers = [x[1] for x in cursor.execute("PRAGMA table_info(PlaceAttributes)").fetchall()]
...
def local_complete_place(place:str,state:str):
	with sqlite3.connect("census.db") as conn:
		cursor = conn.cursor()
		return dict(zip(place_headers, cursor.execute("SELECT * FROM PlaceAttributes WHERE state = ? AND place = ?", (int(state), int(place))).fetchone()))

@app.get('/local/complete/county')
def local_complete_county(county:str,state:str):
	with sqlite3.connect("census.db") as conn:
		cursor = conn.cursor()
		return dict(zip(county_headers, cursor.execute("SELECT * FROM Costs WHERE state = ? AND county = ?", (int(state), int(county))).fetchone()))
